...
describe('StateStore', () => {
  const stateDir = `/tmp/browse-state-test-${Date.now()}`;
  ...
  test('set with JSON object value', () => {
    const store = new StateStore(stateDir);
  ...
  test('list returns all entries', () => {
    ...
    store.set('a', 1);
    store.set('b', 2);
    expect(store.list()).toEqual({ a: 1, b: 2 });
    store.cleanup();
  });
  ...
  test('persistence: set, create new store, get', () => {
    const store1 = new StateStore(stateDir);
    store1.set('persist', { data: 'survives' });

    // Create a second store from the same directory — should load persisted data
    const store2 = new StateStore(stateDir);
    expect(store2.get('persist')).toEqual({ data: 'survives' });
    store2.cleanup();
  });

  test('dot-separated namespaced keys', () => {
    const store = new StateStore(stateDir);
    store.set('eng-review.test-plan', 'plan-content');
    store.set('office-hours.findings', ['item1', 'item2']);
    expect(store.get('eng-review.test-plan')).toBe('plan-content');
    expect(store.get('office-hours.findings')).toEqual(['item1', 'item2']);
    store.cleanup();
  });

  test('flush writes JSON file to disk', () => {
    const store = new StateStore(stateDir);
    store.set('disk', 'check');
    const filePath = path.join(stateDir, 'browse-state.json');
    expect(fs.existsSync(filePath)).toBe(true);
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    expect(raw.disk).toBe('check');
    store.cleanup();
  });

  test('cleanup removes backing file', () => {
    const store = new StateStore(stateDir);
    store.set('temp', 'data');
...
// ─── State Command (via handleMetaCommand) ──────────────────────

describe('State command', () => {
  const stateDir = `/tmp/browse-state-cmd-test-${Date.now()}`;
  ...
  test('state set returns OK', async () => {
    const result = await handleMetaCommand('state', ['set', 'key1', '"hello"'], bm, async () => {}, store);
    expect(result).toBe('OK');
  });

  test('state get returns the value', async () => {
    const result = await handleMetaCommand('state', ['get', 'key1'], bm, async () => {}, store);
    expect(result).toBe('"hello"');
  });

  test('state get nonexistent returns null', async () => {
    const result = await handleMetaCommand('state', ['get', 'missing'], bm, async () => {}, store);
    expect(result).toBe('null');
  });

  test('state set with JSON object', async () => {
    const result = await handleMetaCommand('state', ['set', 'obj', '{"a":1,"b":[2,3]}'], bm, async () => {}, store);
    expect(result).toBe('OK');
    const got = await handleMetaCommand('state', ['get', 'obj'], bm, async () => {}, store);
    expect(JSON.parse(got)).toEqual({ a: 1, b: [2, 3] });
  });

  test('state set with JSON value containing spaces', async () => {
    const result = await handleMetaCommand('state', ['set', 'spaced', '{"msg":', '"hello', 'world"}'], bm, async () => {}, store);
    expect(result).toBe('OK');
    const got = await handleMetaCommand('state', ['get', 'spaced'], bm, async () => {}, store);
    expect(JSON.parse(got)).toEqual({ msg: 'hello world' });
  });

  test('state delete existing key', async () => {
    await handleMetaCommand('state', ['set', 'del-me', '"bye"'], bm, async () => {}, store);
    const result = await handleMetaCommand('state', ['delete', 'del-me'], bm, async () => {}, store);
    expect(result).toBe('Deleted');
  });

  test('state delete nonexistent key', async () => {
    const result = await handleMetaCommand('state', ['delete', 'no-such-key'], bm, async () => {}, store);
    expect(result).toBe('Key not found');
  });

  test('state list shows all entries', async () => {
    store.clear();
    await handleMetaCommand('state', ['set', 'x', '1'], bm, async () => {}, store);
    await handleMetaCommand('state', ['set', 'y', '2'], bm, async () => {}, store);
    const result = await handleMetaCommand('state', ['list'], bm, async () => {}, store);
    const parsed = JSON.parse(result);
    expect(parsed.x).toBe(1);
    expect(parsed.y).toBe(2);
  });

  test('state clear removes everything', async () => {
    await handleMetaCommand('state', ['set', 'a', '"val"'], bm, async () => {}, store);
    const result = await handleMetaCommand('state', ['clear'], bm, async () => {}, store);
    expect(result).toBe('Cleared');
    const list = await handleMetaCommand('state', ['list'], bm, async () => {}, store);
    expect(JSON.parse(list)).toEqual({});
  });

  test('state with no subcommand throws', async () => {
    try {
      await handleMetaCommand('state', [], bm, async () => {}, store);
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('Usage');
    }
  ...
  test('state set with no key throws', async () => {
    try {
      await handleMetaCommand('state', ['set'], bm, async () => {}, store);
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('Usage');
    }
  ...
  test('state set with invalid JSON throws', async () => {
    try {
      await handleMetaCommand('state', ['set', 'key', '{bad json}'], bm, async () => {}, store);
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('Invalid JSON');
    }
  ...
  test('state get with no key throws', async () => {
    try {
      await handleMetaCommand('state', ['get'], bm, async () => {}, store);
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('Usage');
    }
  ...
  test('state delete with no key throws', async () => {
    try {
      await handleMetaCommand('state', ['delete'], bm, async () => {}, store);
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('Usage');
    }
  ...
  test('state without store throws', async () => {
    try {
      await handleMetaCommand('state', ['list'], bm, async () => {});
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain('State store not available');
    }
  });

  test('state with dot-separated namespaced keys', async () => {
    store.clear();
    await handleMetaCommand('state', ['set', 'eng-review.test-plan', '"my plan"'], bm, async () => {}, store);
    const result = await handleMetaCommand('state', ['get', 'eng-review.test-plan'], bm, async () => {}, store);
    expect(JSON.parse(result)).toBe('my plan');
  });
});
