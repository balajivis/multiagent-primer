...
import { isReadCommand, isWriteCommand, isMetaCommand } from './commands';
  ...
  switch (command) {
    ...
    case 'chain': {
      ...
      for (const cmd of commands) {
        ...
        try {
          let result: string;
          if (isWriteCommand(name))       result = await handleWriteCommand(name, cmdArgs, bm);
          else if (isReadCommand(name))   result = await handleReadCommand(name, cmdArgs, bm);
          else if (isMetaCommand(name))   result = await handleMetaCommand(name, cmdArgs, bm, shutdown, stateStore);
    ...
    // ─── State ──────────────────────────────────────────
    case 'state': {
      if (!stateStore) throw new Error('State store not available');
      const sub = args[0];
      switch (sub) {
        case 'get': {
          if (!args[1]) throw new Error('Usage: state get <key>');
          const value = stateStore.get(args[1]);
          return JSON.stringify(value ?? null);
        }
        case 'set': {
          if (!args[1] || args[2] === undefined) throw new Error('Usage: state set <key> <json>');
          let value: any;
          try {
            value = JSON.parse(args.slice(2).join(' '));
          } catch {
            throw new Error('Invalid JSON value. Usage: state set <key> <json>');
          }
          stateStore.set(args[1], value);
          return 'OK';
        }
        case 'delete': {
          if (!args[1]) throw new Error('Usage: state delete <key>');
          const deleted = stateStore.delete(args[1]);
          return deleted ? 'Deleted' : 'Key not found';
        }
        case 'list': {
          return JSON.stringify(stateStore.list());
        }
        case 'clear': {
          stateStore.clear();
          return 'Cleared';
        ...
        default:
          throw new Error('Usage: state get <key> | state set <key> <json> | state delete <key> | state list | state clear');
      }
    }
