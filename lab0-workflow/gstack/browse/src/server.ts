...
import { COMMAND_DESCRIPTIONS, isReadCommand, isWriteCommand, isMetaCommand } from './commands';
import type { CommandRequest, CommandErrorResponse } from './command-types';
...
export type { CommandRequest, CommandErrorResponse };
...
async function handleCommand(body: CommandRequest): Promise<Response> {
  const { command, args = [] } = body;

  if (!command) {
    return new Response(JSON.stringify({ error: 'Missing "command" field' } satisfies CommandErrorResponse), {
      status: 400,
  ...
  try {
    ...
    if (isReadCommand(command)) {
    ...
    } else if (isMetaCommand(command)) {
      result = await handleMetaCommand(command, args, browserManager, shutdown, stateStore);
      ...
      } satisfies CommandErrorResponse), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (hint) errorMsg += '\n' + hint;
    return new Response(JSON.stringify({ error: errorMsg } satisfies CommandErrorResponse), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
