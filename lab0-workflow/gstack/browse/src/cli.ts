...
async function sendCommand(state: ServerState, command: string, args: string[], retries = 0): Promise<void> {
  const payload: CommandRequest = { command, args };
  ...
  try {
    ...
    if (resp.ok) {
      ...
      try {
        const err: CommandErrorResponse = JSON.parse(text);
...
State:          state get <key> | state set <key> <json>
                state delete <key> | state list | state clear
